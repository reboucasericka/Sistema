...
namespace Sistema.Areas.Admin.Controllers
{
    ...
    public class AdminProfessionalController : Controller
    {
        ...
        public AdminProfessionalController(SistemaDbContext context, IConverterHelper converterHelper, IUserHelper userHelper, IImageHelper imageHelper)
        {
            _context = context;
        ...
        public async Task<IActionResult> Create(AdminProfessionalCreateViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.file != null && model.file.Length > 0)
                {
                    ...
                    model.ImageId = await _imageHelper.UploadImageAsync(model.file, "professionals");
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, AdminProfessionalEditViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (model.file != null && model.file.Length > 0)
                    {
                        ...
                        {
                            _imageHelper.DeleteImage(professional.ImageId.ToString(), "professionals");
                        }

                        // Upload new image
                        professional.ImageId = Guid.Parse(await _imageHelper.UploadImageAsync(model.file, "professionals") ?? Guid.Empty.ToString());
                    }
