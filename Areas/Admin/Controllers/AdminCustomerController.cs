...
namespace Sistema.Areas.Admin.Controllers
{
    ...
    public class AdminCustomerController : Controller
    {
        ...
        public AdminCustomerController(SistemaDbContext context, IConverterHelper converterHelper, IUserHelper userHelper, IImageHelper imageHelper)
        {
            _context = context;
        ...
        public async Task<IActionResult> Create(AdminCustomerCreateViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.file != null && model.file.Length > 0)
                {
                    ...
                    {
                        ModelState.AddModelError("file", "Apenas arquivos JPG, PNG, GIF e WebP são permitidos.");
                        return View(model);
                    }
                    ...
                    {
                        ModelState.AddModelError("file", "O tamanho do arquivo não pode exceder 2MB.");
                        return View(model);
                    }

                    model.ImageId = await _imageHelper.UploadImageAsync(model.file, "customers");
                }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, AdminCustomerEditViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (model.file != null && model.file.Length > 0)
                    {
                        ...
                        if (customer.ImageId != Guid.Empty)
                        {
                            _imageHelper.DeleteImage(customer.ImageId.ToString(), "customers");
                        }

                        // Upload new image
                        customer.ImageId = Guid.Parse(await _imageHelper.UploadImageAsync(model.file, "customers") ?? Guid.Empty.ToString());
                    }
